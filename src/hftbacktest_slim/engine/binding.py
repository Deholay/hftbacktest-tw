...
def package_library_candidates() -> tuple[Path, ...]:
    ...
    extension_filename = (
        f"hbt_slim{extension_suffix}"
        if isinstance(extension_suffix, str) and extension_suffix
        else None
    )
    directories = (package_root / "_native", package_root / "native", package_root)
    names = tuple(
        name for name in (LIBRARY_FILENAME, extension_filename) if name is not None
    )
...
def resolve_library_path(
    ...
    raise NativeLibraryNotFoundError(
        "slim native library was not found; reinstall the package from a supported "
        "wheel, or build from source with 'cargo build --workspace --release'. "
        f"An alternate artifact can be selected with {LIBRARY_ENVIRONMENT_VARIABLE}. "
        f"Checked: {rendered}"
    )
